...
En esta sección se detallan las decisiones de diseño orientado a objetos para la implementación de los componentes de software asociados a cada *Bounded Context* identificado en la solución. Con el objetivo de garantizar una arquitectura de software con bajo acoplamiento, alta cohesión y una clara separación de responsabilidades, la estructura interna de cada contexto se ha modelado bajo el enfoque de **Domain-Driven Design (DDD)** y **Clean Architecture**, organizando los componentes en tres capas tácticas fundamentales:

1. **Capa de Dominio (*Domain Layer*):**
  * **Propósito:** Constituye el núcleo central de la aplicación y encapsula las reglas de negocio puras, independientes de cualquier tecnología, base de datos o marco de trabajo.
  * **Componentes:**
    * **Entidades y Agregados (*Entities & Aggregate Roots*):** Clases concretas y abstractas que representan los conceptos principales del negocio, definiendo sus atributos privados, métodos de comportamiento público e identidades unívocas.
    * **Objetos de Valor y Enumeraciones (*Value Objects & Enums*):** Conjuntos fijos de constantes y tipos strictly definidos que restringen los estados válidos del sistema.
    * **Servicios de Dominio (*Domain Services*):** Encapsulan lógica de negocio que involucra múltiples entidades o reglas transversales que no pertenecen a una única entidad.
    * **Contratos de Repositorio (*Repository Interfaces*):** Interfaces (`IRepository`) que definen las operaciones de persistencia requeridas por el dominio, aplicando el principio de inversión de dependencias.

2. **Capa de Aplicación (*Application Layer*):**
  * **Propósito:** Orquesta la ejecución de los casos de uso y coordina los flujos de trabajo del sistema, actuando como intermediario entre la infraestructura exterior y el dominio [1].
  * **Componentes:**
    * **Servicios de Aplicación (*Application Services*):** Clases que reciben las peticiones de los usuarios o sistemas clientes, invocan los contratos del dominio e interactúan con los servicios externos sin contener lógica de negocio directa.

3. **Capa de Infraestructura (*Infrastructure Layer*):**
  * **Propósito:** Provee el soporte técnico y las implementaciones concretas para la persistencia de datos y la comunicación con servicios externos.
  * **Componentes:**
    * **Implementación de Repositorios (*Repository Implementations & DbContext*):** Clases concretas que implementan las interfaces del dominio para interactuar con la base de datos relacional.
    * **Servicios y APIs Externas (*External APIs & Services*):** Adaptadores de integración para el consumo de servicios de Inteligencia Artificial (generación de perfiles, simulaciones y reportes) y servicios de terceros (pasarelas de pago y envío de notificaciones/correos).

---

### 4.7.1. Class Diagrams

#### 4.7.1.1. User Registration & Profile Context
* **Propósito:** Se encarga de la autenticación, la recuperación de credenciales y la vinculación de estudiantes con sus psicólogos asignados.
...
<div align="center">
  <img src="images/diagrams/bc-01-user-registration.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-02-search.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-03-vocational-evaluation.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-04-career-exploration.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-05-student-monitoring.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-06-reports.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-07-payment-subscription.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-08-scheduling-session.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-09-community-chat.png" alt="user registration"><br><br>
</div>
...
<div align="center">
  <img src="images/diagrams/bc-10-system-notifications.png" alt="user registration"><br><br>
</div>
...
**Entidades de VocaFy y sus Relaciones**

El esquema físico traduce los conceptos del dominio de orientación vocacional en tablas relacionales estructuradas:

* **Entidades:** Representan los conceptos clave de la aplicación (usuarios, estudiantes, psicólogos, pruebas vocacionales, perfiles vocacionales, carreras, sesiones, suscripciones, publicaciones comunitarias y notificaciones).
* **Relaciones e Integridad Referencial:**
  * **Relación Uno a Uno:**
    * **Usuarios con Estudiantes y Psicólogos:** Implementa la herencia de cuentas. Cada registro en estudiantes o psicólogos comparte el identificador de usuario como clave primaria y foránea respecto a la tabla base de usuarios.
    * **Pruebas Vocacionales con Perfiles Vocacionales:** Una prueba vocacional completada genera de forma biunívoca exactamente un perfil vocacional a través de una clave foránea única.
  * **Relación Uno a Muchos:**
    * Un estudiante realiza múltiples evaluaciones a lo largo del tiempo.
    * Un psicólogo gestiona múltiples sesiones de orientación y registra diversas observaciones clínicas.
    * Una suscripción acumula múltiples intentos o cobros en transacciones de pago.
  * **Relación Muchos a Muchos:**
    * Los perfiles vocacionales se relacionan con múltiples opciones del catálogo de carreras. Esta asociación se resuelve en el esquema físico mediante una tabla intermedia que almacena los identificadores de la carrera y del perfil, junto con la métrica de porcentaje de compatibilidad.
  
<div align="center">
  <img src="images/diagrams/database-diagram.png" alt="user registration"><br><br>
</div>
...
La gestión de configuración del software de NextPath permite organizar y controlar los artefactos generados durante el desarrollo, mantener la trazabilidad de los cambios y facilitar el trabajo colaborativo entre los integrantes del equipo.
...
<tr>
<td>GitHub Issues</td>
<td>Registro, organización y seguimiento de requerimientos, actividades y problemas identificados durante el desarrollo del proyecto.</td>
<td>https://github.com/features/issues</td>
</tr>
<tr>
<td>Markdown</td>
<td>Elaboración y mantenimiento de documentación estructurada de requerimientos y otros artefactos del proyecto dentro de los repositorios.</td>
...
<tr>
<td>UXPressia</td>
<td>Elaboración de User Personas, Empathy Maps, Journey Maps e Impact Maps para representar las características, necesidades y experiencias de los usuarios.</td>
<td>https://uxpressia.com/</td>
</tr>
<tr>
<td>Figma</td>
<td>Diseño de Wireframes, Mock-ups y Prototypes de las interfaces de NextPath.</td>
<td>https://www.figma.com/</td>
</tr>
<tr>
<td>FigJam</td>
<td>Elaboración colaborativa de Wireflows, User Flows y otros esquemas de interacción.</td>
<td>https://www.figma.com/figjam/</td>
</tr>
<tr>
<td>Miro</td>
<td>Elaboración colaborativa de diagramas y actividades de EventStorming.</td>
...
<td>Editor de código fuente utilizado para desarrollar y mantener los productos de software del proyecto.</td>
<td>https://code.visualstudio.com/</td>
</tr>
<tr>
<td>HTML5</td>
<td>Lenguaje de marcado utilizado para estructurar el contenido de la Landing Page y las interfaces web.</td>
<td>https://developer.mozilla.org/en-US/docs/Web/HTML</td>
</tr>
<tr>
<td>CSS3</td>
<td>Lenguaje utilizado para definir la presentación visual, estilos y comportamiento responsive de las interfaces.</td>
<td>https://developer.mozilla.org/en-US/docs/Web/CSS</td>
</tr>
<tr>
<td>JavaScript</td>
<td>Lenguaje utilizado para implementar la interacción y comportamiento dinámico de las interfaces web.</td>
...
Para la gestión del código fuente de NextPath se utilizará **Git** como sistema de control de versiones y **GitHub** como plataforma para el almacenamiento remoto de los repositorios, colaboración entre los integrantes del equipo y gestión de las diferentes versiones del proyecto.
...
<td>VocaFyTeam/NextPath-Report</td>
<td>Almacenar la documentación y los artefactos correspondientes al Project Report de NextPath.</td>
</tr>
<tr>
<td>Landing Page</td>
...
https://github.com/VocaFyTeam/NextPath-Report
...
La estructura principal de ramas será:
...
<td>Contiene las versiones estables y listas para producción de los productos de software.</td>
<td><code>main</code></td>
</tr>
<tr>
<td><code>develop</code></td>
<td>Rama principal de integración donde se incorporan las funcionalidades desarrolladas antes de generar una versión estable.</td>
<td><code>develop</code></td>
</tr>
<tr>
<td><code>feature/&lt;name&gt;</code></td>
<td>Ramas utilizadas para desarrollar nuevas funcionalidades o realizar cambios específicos.</td>
<td><code>feature/landing-page</code></td>
</tr>
<tr>
...
2. Verificar que los archivos HTML, CSS, JavaScript, imágenes y demás recursos necesarios se encuentren correctamente organizados.
3. Realizar las pruebas correspondientes en el entorno local.
4. Realizar el commit de los cambios utilizando la convención de **Conventional Commits**.
5. Realizar el push de los cambios hacia el repositorio remoto.
6. Configurar GitHub Pages como mecanismo de publicación.
7. Seleccionar la rama correspondiente para el deployment.
8. Seleccionar la carpeta que contiene los archivos que serán publicados.
9. Esperar la generación del sitio por parte de GitHub Pages.
10. Acceder a la URL pública generada.
